...

{
          ...
          if (len < sizeof(sr_ip_hdr_t) + sizeof(sr_ethernet_hdr_t)) /* Length check. Must be at least bigger than ip + eth header.*/
          {
              printf("%s\n", "Packet length is too small." );
              return;
          }
          else /* checksum verification if length check passes. */
          {
            ...
            printf("%s\n", "Packet length is ok." );
              ...
              else /* Checksum for IP header passed */
              {
                ...
                if (wasPacketSentToRouter(sr, ipHead) == 1) /* It was destined to one of router's interfaces. */
                {
                    ...
                    {
                        /* TCPUDPHandler(sr, packet, ipHead, interface, 3, 3); */
